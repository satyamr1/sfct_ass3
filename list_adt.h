...
public:
	...
	void updateFlow(graph G){           // Updates the residual graph at each iteration
        int cf = findMin(G);
	    for(int i=0;i<sp;i++){
            if(G.getCapacity(arr[i]->u,arr[i]->v) != 0)
                G.setFlow(arr[i]->u,arr[i]->v,G.getFlow(arr[i]->u,arr[i]->v)+cf);
            else
                G.setFlow(arr[i]->v,arr[i]->u,G.getFlow(arr[i]->u,arr[i]->v)-cf);

                G.setCapacity(arr[i]->u,arr[i]->v,G.getCapacity(arr[i]->u,arr[i]->v)-cf);
                G.setCapacity(arr[i]->v,arr[i]->u,G.getCapacity(arr[i]->v,arr[i]->u)+cf);
		}
	}
	bool isEmpty(){
